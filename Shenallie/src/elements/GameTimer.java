...
//		if (currentTime - this.previousTimeMonster >= 5000*1000000) {
		...
		if (monsterElapsedTime >= SPAWNDELAY_MONSTERS) {
			int[] monsterType = {11, 12, 13, 111, 121, 131, 21, 22, 23, 31, 32, 33};
			...
			for (int i = monsterArrayList.size(); i < NUM_MONSTER; i++) {
				...
				int health = 0;
				if (randomType == 11 || randomType == 21 || randomType == 31) {
					health = 30;
				} else if (randomType == 12 || randomType == 22 || randomType == 32) {
					health = 50;
				} else {
					health = 70;
				}
				
				Monster monster = new Monster(spawnX, spawnY, randomType, health);
...
//  Render monster
	...
	public void animationMonster(long currentTime) {		
		for(Monster monster : monsterArrayList){
			monster.animation(currentTime, player2);
		}
	}
